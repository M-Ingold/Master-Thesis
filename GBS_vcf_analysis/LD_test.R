...
for (i in 1:12) {
  ...
  like_lds[[i]] <- mldest(geno = larray, K = ploidy, type = "comp", nc=11)
}
...
ld_dfs <- list()
for (n in 1:12) {
  df <- data.frame(i=map$Position[like_lds[[n]]$i], j=map$Position[like_lds[[n]]$j], r2=like_lds[[n]]$r2, Dprime=like_lds[[n]]$Dprime)
...
for (i in 1:12) {
  ...
  newrsq <- ( (10+rec*ld_df$dist) / ( (2+rec*ld_df$dist) * (11+rec*ld_df$dist) ) ) * ( 1 + ( (3+rec * ld_df$dist) * (12+12*rec*ld_df$dist + (rec*ld_df$dist)^2) ) / (n*(2+rec*ld_df$dist) * (11+rec*ld_df$dist) ) )
  nf <- data.frame(ld_dfs[[i]]$dist, newrsq)
  newfiles[[i]] <- nf[order(nf$ld_df.dist),]
}
...
for (i in 1:12) {
  ...
  halfdecay = 0.5 * maxld
  halfdecaydist[[i]] <- newfile$ld_df.dist[which.min(abs(newfile$newrsq-halfdecay))]
  tenthdecay = 0.1
  tenthdecaydist[[i]] <- newfile$ld_df.dist[which.min(abs(newfile$newrsq-tenthdecay))]
  
  # plotting the values
  par(mar = c(5, 5, 1, 1)) 
  plot(ld_df$dist, ld_df$r2, pch=".", cex=2, xlab="Distance (bp)", main=paste0("Chromosome",i), ylab=expression(LD ~ (r^2)), col="grey")
...
spline <- data.frame(ld_df$dist, predict(spline_fit5))

maxld <- max(spline[2])
halfdecay = 0.5 * maxld
halfdecaydist <- spline[subset(spline[2])<halfdecay,][1,1]

tenthdecay = 0.1
tenthdecaydist <- spline[subset(spline[2])<tenthdecay,][1,1]

test <- rq(r2 ~ bs(dist, df=5, degree = 4), tau = 0.9, data = ld_df)
lines(ld_df$dist, predict(test,), lty = 1, col = "black", lwd = 2)
